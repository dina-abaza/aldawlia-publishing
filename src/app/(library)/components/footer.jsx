...
const Footer = () => {
  ...
  // وظيفة لتنسيق رابط الواتساب (حسب الرد الجديد للباك إند)
  const formatWhatsappUrl = (url) => {
    if (!url || url === "") return "#";
    // إذا كان الباك إند يرسل رابطاً كاملاً مثل https://wa.me/.. نستخدمه كما هو
    // وإذا كان يرسل رقماً فقط، دالة formatExternalUrl ستتعامل معه
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        // التعديل هنا: الوصول لـ res.data.data حسب مثال الاستجابة الخاص بكِ
    ...
    <footer className="bg-sky-950 border-t-8 border-amber-600 pt-12 pb-8 mt-12 shadow-2xl relative text-white" dir="rtl">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div>
            ...
            <div className="flex flex-col gap-6 items-center md:items-start">
              ...
              <div className="flex flex-col gap-4 items-center md:items-start text-sm font-medium">
                <a href={`mailto:${contactEmail}`} className="flex items-center gap-3 hover:text-amber-600 transition-colors">
                  <Mail size={16} className="text-amber-600" />
                  <span>{contactEmail}</span>
                </a>

                {settings.phone && (
                  <a href={`tel:${settings.phone}`} className="flex items-center gap-3 hover:text-amber-600 transition-colors" dir="ltr">
        ...
        <div className="text-center text-sm font-light uppercase tracking-widest flex flex-col md:flex-row justify-center items-center gap-2">
          {/* عرض النص القادم من الباك إند أو النص الافتراضي */}
